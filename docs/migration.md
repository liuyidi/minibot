...
> **落盘路径：** [`docs/minibot-fastapi-migration.md`](./minibot-fastapi-migration.md)
...
> **配套：**
> - 现状基线：[`minibot-current-status.md`](./status.md)
> - Phase 短计划：见 §短计划清单 / `docs/phase-*.md`
...
- **合同：** [`client-api.md`](./client-api.md)（统一大纲；旧 nanobot 手册已删除）
...
详见 [`minibot-current-status.md`](./status.md)（若与 checklist 冲突，以 checklist 为准）。
...
| Settings / 导入向导 | **6 余量** | ✅ | Anthropic/registry、nanobot 导入（Azure/Bedrock stub） |
| Trace `used_provider` + toast | **6.5** | ✅ | FallbackProvider；`provider_switched`；Runtime 故障注入调试 |
...
**短计划：** [`phase-1.5-subagent.md`](./phases/phase-1.5-subagent.md)  
**MSV=1.5**（可选切换项）
...
**短计划：** [`phase-2-streaming.md`](./phases/phase-2-streaming.md) ✅ 已落地
...
**短计划：** 开工前扩 [`phase-1.5-subagent.md`](./phases/phase-1.5-subagent.md) 的 1.5B 节
...
**短计划：** [`phase-3a-context.md`](./phases/phase-3a-context.md)  
**MSV=3a**
...
| **6.2** settings 路由 | 按 [`client-api.md`](./client-api.md) 实装：model-configurations CRUD、provider update / provider-models discovery、usage / version-check、web-search / image-generation / transcription / network-safety、cli-apps install\|update\|uninstall\|test | M-API 🔌、M-Cfg 🔧 |
...
**目标：** 生产可用性；主模型挂了自动走备胎。
**MSV=6.5** · 详见 [`phase-6.5-fallback.md`](./phases/phase-6.5-fallback.md)

| 子步骤 | 改什么 | 模块影响 | 状态 |
|--------|--------|----------|------|
| **6.5.1** FallbackProvider | `providers/fallback.py`：包一层 `FallbackProvider`，捕获 429/5xx/超时/error 后按序试 | M-Prov 🆕 | ✅ |
| **6.5.2** modelPresets 加 fallback | schema 加 `fallback: [presetId, ...]`；`build_provider_chain` | M-Cfg ⚠️ | ✅ |
| **6.5.3** trace 记录 | Runner/Loop 记 `used_provider` / `used_preset` | M-Loop 🔧、M-Runner 🔧 | ✅ |
| **6.5.4** WS 事件 | `provider_switched`（Chat toast）+ `runtime.html` stats | M-API 🔌、M-Loop 🔧 | ✅ |
...
- `webui` 默认 API 指向 minibot；`docs/getting-started.md` 改走 minibot
...
**MSV=10**（可选，不阻塞切换）
**详细计划：** [`phase-10-langfuse.md`](./phases/phase-10-langfuse.md)
...
| 短计划文件 | 状态 | 触发时机 |
|------------|------|----------|
| [`phase-1.5-subagent.md`](./phases/phase-1.5-subagent.md) | ✅ | 1.5A sync；2.5 async 待扩 |
| [`phase-3a-context.md`](./phases/phase-3a-context.md) | ✅ | 已完成 |
| [`phase-3b-memory-skills.md`](./phases/phase-3b-memory-skills.md) | ✅ | 已完成 |
...
- [x] **Phase 1.5A**：sync `spawn` 【MSV=1.5】详见 [`phase-1.5-subagent.md`](./phases/phase-1.5-subagent.md)
- [x] **Phase 3a**：context + compaction 【MSV=3a】详见 [`phase-3a-context.md`](./phases/phase-3a-context.md)
- [x] **Phase 3b**：memory + skills 【MSV=3b】详见 [`phase-3b-memory-skills.md`](./phases/phase-3b-memory-skills.md)
...
- [x] **① Phase 6 余量（核心）**：registry + Anthropic Messages + nanobot 导入 MVP 【MSV=6】详见 [`phase-6-providers.md`](./phases/phase-6-providers.md)
  - Azure / Bedrock / OAuth 仍为 stub
- [x] **③ Phase 6.5**：Fallback / Retry 链 【MSV=6.5】详见 [`phase-6.5-fallback.md`](./phases/phase-6.5-fallback.md)
...
- **文档同步**：改合同时同步 `docs/client-api.md`；进度单源在本计划 checklist
