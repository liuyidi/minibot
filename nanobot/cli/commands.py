...
def _resolve_webui_runtime_surface(cli_value: str) -> str:
    """Resolve WebUI surface from CLI / env. Explicit CLI native wins."""
    env = (os.environ.get("NANOBOT_WEBUI_RUNTIME_SURFACE") or "").strip().lower()
    cli = (cli_value or "").strip().lower()
    if cli in {"native", "desktop"}:
        return "native"
    if env in {"native", "desktop"}:
        return "native"
    if cli in {"browser", "web", ""}:
        return "browser"
    console.print(
        f"[red]Error: invalid --runtime-surface {cli_value!r}; "
        "expected 'browser' or 'native'[/red]"
    )
    raise typer.Exit(1)


def _with_websocket_channel(
    config: Config,
    *,
    port: int | None,
    enabled: bool,
) -> Config:
    """Return a copy of config with the websocket channel enabled/updated."""
...
@app.command()
def gateway(
    port: int | None = typer.Option(None, "--port", "-p", help="Gateway health port"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Verbose output"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
    runtime_surface: str = typer.Option(
        "browser",
        "--runtime-surface",
        help="WebUI runtime surface: browser or native (desktop host)",
        case_sensitive=False,
    ),
    websocket_port: int | None = typer.Option(
        None,
        "--websocket-port",
        help="Enable the websocket channel and bind WebUI on this port",
    ),
):
    ...
    surface = _resolve_webui_runtime_surface(runtime_surface)
    if websocket_port is not None:
        cfg = _with_websocket_channel(cfg, port=websocket_port, enabled=True)
    elif surface == "native":
        # Desktop hosts always need the websocket/WebUI channel.
        cfg = _with_websocket_channel(cfg, port=None, enabled=True)
    _run_gateway(cfg, port=port, webui_runtime_surface=surface)
