...
export function AddSkillDialog({
  ...
  const applyFile = useCallback(async (file: File | undefined | null) => {
    ...
    setName(skillNameFromFile(file.name));
    setError(null);
  }, []);
  ...
  const submit = async () => {
    if (!markdown.trim()) {
      setError(
        t("settings.skills.uploadRequired", {
          defaultValue: "Please upload a SKILL.md file first.",
        }),
      );
      return;
    }
      ...
      <DialogContent className="max-w-xl gap-5 rounded-2xl p-6">
        <DialogHeader>
          ...
          <DialogDescription>
            {t("settings.skills.addSkillHint", {
              defaultValue: "Upload a SKILL.md. It is saved under the current workspace skills/ folder.",
            })}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-3">
          ...
          <button
            ...
            onDrop={onDrop}
            className={cn(
              "flex w-full cursor-pointer flex-col items-center justify-center gap-3 rounded-xl border border-dashed px-4 py-10 text-center transition-colors",
              dragging
          ...
          <ul className="space-y-1 px-0.5 text-[12px] leading-5 text-muted-foreground">
            <li>
              {t("settings.skills.uploadRuleMarkdown", {
                defaultValue: "Upload a .md / SKILL.md file.",
              })}
            </li>
            <li>
