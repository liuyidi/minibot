...
describe("MinibotClient", () => {
  ...
  it("can swap the socket factory when the runtime URL changes", () => {
    ...
    client.close();
    client.updateUrl("minibot-host://engine/", hostFactory);
    client.connect();

    expect(hostFactory).toHaveBeenCalledWith("minibot-host://engine/");
    expect(lastSocket().url).toBe("host:minibot-host://engine/");
  });
  ...
  it("uses the host socket bridge for native host URLs", async () => {
    ...
    expect(openSocket).toHaveBeenCalledWith("minibot-host://engine/");
