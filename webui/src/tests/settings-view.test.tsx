...
describe("SettingsView", () => {
  ...
  it("shows a slim overview without token usage or capability rows", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn(async (input: RequestInfo | URL) => {
        const url = String(input);
    ...
    await waitFor(() =>
      expect(
        fetchMock.mock.calls.some(
          ([input]) => String(input) === "/api/settings/provider-models?provider=deepseek",
        ),
      ).toBe(true),
    );
    await waitFor(() =>
      expect(
        fetchMock.mock.calls.some(([input]) => String(input) === "/api/settings/update"),
      ).toBe(true),
    );
  });
