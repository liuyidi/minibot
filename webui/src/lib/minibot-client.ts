...
const WS_OPEN = 1;
const WS_CLOSING = 2;
const HOST_SOCKET_URL_PREFIX = "minibot-host://";

function isHostSocketUrl(url: string): boolean {
  return url.startsWith(HOST_SOCKET_URL_PREFIX);
}

function createDefaultSocket(url: string): WebSocket {
  if (isHostSocketUrl(url)) {
 ...
 * - **Production**: off unless ``localStorage.setItem('minibot_debug_ws','1')`` (or ``true``).
 * - **Silence anywhere**: ``localStorage.setItem('minibot_debug_ws','0')`` (or ``false`` / ``off``).
...
function wsInboundDebugEnabled(): boolean {
  ...
  try {
    ...
    const raw = ls?.getItem("minibot_debug_ws")?.trim().toLowerCase() ?? "";
...
export interface MinibotClientOptions {
  url: string;
  reconnect?: boolean;
...
export class MinibotClient {
  ...
  constructor(private options: MinibotClientOptions) {
  ...
  private handleMessage(ev: MessageEvent): void {
    ...
    try {
      ...
      if (wsInboundDebugEnabled()) {
        ...
        console.warn(
          "[minibot ws inbound] invalid JSON",
    ...
    if (wsInboundDebugEnabled()) {
      console.log("[minibot ws inbound]", summarizeInboundWsPayload(parsed));
    }
  ...
  private dispatch(chatId: string, ev: InboundEvent): void {
    ...
    q.push(ev);
    const over = q.length - MinibotClient.PENDING_INBOUND_MAX;
    if (over > 0) {
      q.splice(0, over);
    }
  }
