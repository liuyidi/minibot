...
from minibot.utils.media_decode import FileSizeExceeded, save_base64_data_url
from minibot.webui.ingress_policy import (
    ...
    *,
    ...
    def abort(reason: AttachmentRejection) -> AttachmentIngressResult:
        for path in paths:
            try:
                ...
                logger.warning("failed to unlink partial media %s: %s", path, exc)
        return [], reason
    ...
    for item in media:
        ...
        try:
            ...
            logger.warning("media decode failed: %s", exc)
            return abort("decode")
        if saved is None:
            return abort("decode")
        paths.append(saved)
        if not is_video:
            try:
                ...
                logger.warning("failed to stat inbound attachment %s: %s", saved, exc)
                return abort("decode")
