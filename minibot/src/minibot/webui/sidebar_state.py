"""Persisted WebUI sidebar workspace state (Phase 8.3).

UI-only metadata (pin / archive / rename), scoped to ``data_dir/webui/``.
Does not modify agent session JSONL.
"""
...
def webui_sidebar_state_path(data_dir: Path) -> Path:
    return Path(data_dir).expanduser() / "webui" / "sidebar-state.json"
...
def read_webui_sidebar_state(data_dir: Path) -> dict[str, Any]:
    path = webui_sidebar_state_path(data_dir)
    if not path.is_file():
        return default_webui_sidebar_state()
    try:
        if path.stat().st_size > _MAX_STATE_FILE_BYTES:
            logger.warning("webui sidebar state too large, ignoring: %s", path)
            return default_webui_sidebar_state()
        with open(path, encoding="utf-8") as f:
            raw = json.load(f)
    except (OSError, json.JSONDecodeError) as e:
        logger.warning("read webui sidebar state failed %s: %s", path, e)
        return default_webui_sidebar_state()
    return normalize_webui_sidebar_state(raw)


def write_webui_sidebar_state(data_dir: Path, raw: dict[str, Any]) -> dict[str, Any]:
