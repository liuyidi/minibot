...
from minibot.apps.protocol import app_manifest, compact_dict
from minibot.security.workspace_access import is_path_within


def _default_cli_apps_data_dir() -> Path:
    """Per-user (or instance) CLI Apps install-state + registry cache root."""
    ...
    path = Path(root).expanduser() / "cli-apps"
...
class CliAppManager:
    ...
    def __init__(
        ...
        *,
        ...
        self.data_dir = Path(data_dir) if data_dir is not None else _default_cli_apps_data_dir()
    ...
    def _trust_registry(self, app: dict[str, Any]) -> str:
        del app  # catalog trust is always CLI-Anything for current sources
        return "cli-anything"
        ...
        *,
        ...
        remove = compact_dict({
            ...
            "verification": (
                ...
                else ["minibot_state_absent", "managed_paths_absent"]
            ),
        })
        return app_manifest(
            app_id=name,
    ...
    def _fallback_skill(self, app: dict[str, Any]) -> str:
        ...
        description = _catalog_description(app) or f"Use {display} from minibot."
        return f"""---
...
Use this skill when the user asks minibot to operate {display} through its installed CLI app.
...
"""
    ...
    def _with_minibot_skill_note(self, content: str, app: dict[str, Any]) -> str:
        marker = "<!-- minibot-cli-app-note -->"
...
"""
    ...
    def install_skill(self, app: dict[str, Any]) -> Path:
        ...
        content = self._with_minibot_skill_note(content, app)
    ...
    def uninstall(self, name: str) -> dict[str, Any]:
        ...
        if strategy == "bundled" and still_available:
            message = (
                f"Removed {app['display_name']} from minibot. {entry_point} "
                "is still available because it is managed outside minibot."
            )
