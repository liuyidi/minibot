...
from minibot.channels.helpers import redirect_lib_logging
...
class FeishuChannel(BaseChannel):
    ...
    async def _enqueue_pairing(self, sender_id: str) -> None:
        """Record a pending DM pairing request and tell the user to wait."""
        try:
            from minibot.channels.pairing import PairingStore
            from minibot.config.settings import get_settings

            store = PairingStore(get_settings().data_dir)
            item = store.ensure_pending(sender_id, chat_type="p2p")
            self.logger.info(
                "Feishu pairing pending id={} sender={}",
                item.id,
                sender_id,
            )
    ...
    async def start(self) -> None:
        ...
        if not FEISHU_AVAILABLE:
            self.logger.error("SDK not installed. Run: pip install 'minibot[feishu]'")
            return
    ...
    async def _on_message(self, data: P2ImMessageReceiveV1) -> None:
        ...
        try:
            ...
            # Early permission / pairing gate (OpenClaw-style dm_policy).
            if not self.is_allowed(sender_id):
                dm_policy = str(getattr(self, "dm_policy", None) or "allowlist")
                if dm_policy == "open":
                    pass  # fall through — open DMs
                elif chat_type == "p2p" and dm_policy == "pairing":
                    await self._enqueue_pairing(sender_id)
                    return
                else:
                    self.logger.warning(
                        "Access denied for sender {} (policy={})",
                        sender_id,
                        dm_policy,
                    )
                    return
