...
def _write_tauri_package(repo: Path, prefix: str, npm_name: str, cargo_name: str) -> None:
    (repo / f"{prefix}/src-tauri").mkdir(parents=True)
    (repo / f"{prefix}/package.json").write_text(
        json.dumps({"name": npm_name, "version": "1.0.0-beta.2"}, indent=2) + "\n",
        encoding="utf-8",
    )
    (repo / f"{prefix}/package-lock.json").write_text(
        json.dumps(
            {
                "name": npm_name,
                "version": "1.0.0-beta.2",
                "packages": {"": {"name": npm_name, "version": "1.0.0-beta.2"}},
            },
            indent=2,
        )
        + "\n",
    ...
    (repo / f"{prefix}/src-tauri/Cargo.toml").write_text(
        f"""[package]
name = "{cargo_name}"
version = "1.0.0-beta.2"
""",
    ...
    (repo / f"{prefix}/src-tauri/Cargo.lock").write_text(
        f"""[[package]]
name = "{cargo_name}"
version = "1.0.0-beta.2"
""",
    ...
    (repo / f"{prefix}/src-tauri/tauri.conf.json").write_text(
        json.dumps(
            {
                "productName": "minibot",
                "version": "1.0.0-beta.2",
